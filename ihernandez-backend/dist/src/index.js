...
const passwordIsValid = async (plainPassword, storedPassword) => {
    if (!storedPassword) {
        return false;
    }
    if (storedPassword.startsWith("$2a$") || storedPassword.startsWith("$2b$") || storedPassword.startsWith("$2y$")) {
        return bcrypt_1.default.compare(plainPassword, storedPassword);
    }
...
app.post("/api/activities", verifyToken, requireRole(["admin"]), async (req, res) => {
    try {
...
app.put("/api/activities/:id", verifyToken, requireRole(["admin"]), async (req, res) => {
    try {
        const id = Number(req.params.id);
        const { provider_id, city_id, name, description, category, price, duration_minutes, max_capacity } = req.body;
        if (!id || isNaN(id)) {
...
app.delete("/api/activities/:id", verifyToken, requireRole(["admin"]), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id || isNaN(id)) {
            return res.status(400).json({ error: "ID inválido" });
...
/* ================= AI PLAN SUBMISSIONS ================= */
app.post("/api/ai-plan-submissions", async (req, res) => {
    try {
        const { suggestedName, cityId, description, category, suggestedPrice, maxCapacity, durationMinutes, } = req.body;
...
app.get("/api/ai-plan-submissions", verifyToken, requireRole(["admin"]), async (_req, res) => {
    try {
        const result = await pool.query(`
...
app.get("/api/community-ideas", async (_req, res) => {
    try {
        const result = await pool.query(`
            SELECT
                s.id,
...
app.patch("/api/ai-plan-submissions/:id/reject", verifyToken, requireRole(["admin"]), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id || isNaN(id)) {
            return res.status(400).json({ error: "ID inválido" });
...
app.patch("/api/ai-plan-submissions/:id/approve", verifyToken, requireRole(["admin"]), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!id || isNaN(id)) {
            return res.status(400).json({ error: "ID inválido" });
...
app.get("/api/orders", verifyToken, requireRole(["admin"]), async (_req, res) => {
    try {
        ...
        const itemsResult = await pool.query(`
            ...
            ORDER BY bi.id
        `, [bookingIds]);
        const itemsByBooking = itemsResult.rows.reduce((acc, item) => {
...
app.post("/api/orders", verifyToken, requireRole(["customer", "admin"]), async (req, res) => {
    try {
        if (!req.customer) {
            return res.status(401).json({ error: "Usuario no autenticado" });
        }
        ...
        const firstCustomPlan = items.find((item) => item.customPlan?.cityId);
        const activityIds = items
            .map((item) => Number(item.activityId || item.productId))
            .filter((id) => id > 0);
        ...
        for (const item of items) {
            ...
            if (!activityId || activityId <= 0 || quantity <= 0 || unitPrice < 0)
                continue;
            const itemResult = await pool.query(`
                INSERT INTO booking_items (booking_id, activity_id, quantity, unit_price)
                VALUES ($1, $2, $3, $4)
...
app.get("/api/orders/my", verifyToken, requireRole(["customer", "admin"]), async (req, res) => {
    try {
        if (!req.customer) {
            return res.status(401).json({ error: "Usuario no autenticado" });
        }
...
app.patch("/api/orders/:id/status", verifyToken, requireRole(["admin"]), async (req, res) => {
    try {
        ...
        const result = await pool.query("UPDATE bookings SET status = $1 WHERE id = $2 RETURNING *", [status, orderId]);
        if (result.rows.length === 0) {
...
app.get("/api/payments/booking/:booking_id", verifyToken, async (req, res) => {
    try {
