...
app.put("/api/products/:id", verifyToken, requireRole(["admin"]), async (req: Request, res: Response) => {
    const id = Number(req.params.id);
...
app.delete("/api/products/:id", verifyToken, requireRole(["admin"]), async (req: Request, res: Response) => {
    const id = Number(req.params.id);
...
app.put("/api/users/:id", verifyToken, requireRole(["admin"]), async (req: Request, res: Response) => {
    try {
        ...
        const result = await pool.query(
            "UPDATE customers SET email = $1, full_name = $2, role = $3 WHERE id = $4 RETURNING id, email, full_name, username, role",
            [email, name, role, id]
        );
...
app.post("/api/orders", verifyToken, requireRole(["customer"]), async (req: AuthRequest, res: Response) => {
    try {
        ...
        const orderTotal = items.reduce(
            (acc: number, item: any) => acc + (Number(item.unitPrice) || 0) * (Number(item.quantity) || 0),
            0
        );

        const orderResult = await pool.query(
            `INSERT INTO orders (customer_id, status, total, address)
             VALUES ($1, $2, $3, $4)
             RETURNING *`,
            [req.customer.id, "pending", orderTotal, address]
        );
...
app.get("/api/orders/my", verifyToken, requireRole(["customer"]), async (req: AuthRequest, res: Response) => {
    try {
        if (!req.customer) {
            return res.status(401).json({ error: "Usuario no autenticado" });
        }
        ...
        const itemsByOrder = itemsResult.rows.reduce((acc: Record<number, any[]>, item: any) => {
...
app.get("/api/orders", verifyToken, requireRole(["admin", "employee"]), async (_req: Request, res: Response) => {
    try {
        const ordersResult = await pool.query(
...
app.patch("/api/orders/:id/status", verifyToken, requireRole(["admin", "employee"]), async (req: Request, res: Response) => {
    try {
...
app.get("/api/fichajes", verifyToken, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const result = await pool.query(
            `SELECT id, customer_id, tipo, created_at FROM fichajes 
             WHERE customer_id = $1
